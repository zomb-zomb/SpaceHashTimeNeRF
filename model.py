...
from field import SpaceTimeHashingField
...
from field import SpaceTimeHashingField, SpaceTimeDensityField, SpaceTimeDensityFieldWithBase
from ray_samplers import ProposalNetworkSamplerSpatial, UniformSamplerSpatial

@dataclass
class SpaceTimeHashingModelConfig(ModelConfig):
    _target: Type = field(
        default_factory=lambda: SpaceTimeHashingModel
...
class SpaceTimeHashingModel(Model):
    ...
    field: SpaceTimeHashingField

    def __init__(self, config: SpaceTimeHashingModelConfig, **kwargs) -> None:
    ...
    def populate_modules(self):
        ...
        sampler_class = SpaceTimeDensityFieldWithBase
...
@dataclass
class SpaceTimeHashingUniformModelConfig(ModelConfig):
    """Instant NGP Model Config"""

    _target: Type = field(
        default_factory=lambda: SpaceTimeHashingUniformModel
    )  # We can't write `NGPModel` directly, because `NGPModel` doesn't exist yet
    """target class to instantiate"""
    enable_collider: bool = False
    """Whether to create a scene collider to filter rays."""
...
class SpaceTimeHashingUniformModel(Model):
    """Instant NGP model
    ...
    config: SpaceTimeHashingUniformModelConfig
    field: SpaceTimeHashingField

    def __init__(self, config: SpaceTimeHashingUniformModelConfig, **kwargs) -> None:
        super().__init__(config=config, **kwargs)
    ...
    def populate_modules(self):
        ...
        self.sampler = UniformSamplerSpatial(num_samples=self.config.num_uniform_samples, single_jitter=True)
        # renderers
        self.renderer_rgb = RGBRenderer(background_color=self.config.background_color)
        self.renderer_accumulation = AccumulationRenderer()
    ...
    def get_outputs(self, ray_bundle: RayBundle):
        ...
        depth = self.renderer_depth(
            weights=weights, ray_samples=ray_samples, num_rays=num_rays
        )
        accumulation = self.renderer_accumulation(weights=weights, num_rays=num_rays)
